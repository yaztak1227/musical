...
- 表示モードボタンは `Wave`, `Spectrum`, `Circle`, `Peaks`, `Aurora`, `Starfield`, `DNA Helix`, `Dreamflow`, `VU meters`, `Warp Hole` を持つ。`Dreamflow`（日本語名は夢幻流、内部IDは `ink`）は、複数の半透明な水玉の穏やかな周回と周波数連動の膨縮を維持し、固定ペアが近づいた部分だけへ接近度とenergyに応じた短い混色膜を描く。各接点の周囲へ位相の異なる小さな光点を不均一に散らして明滅させ、深く重なった膜は減衰させる。水玉を全面的なリボンへ変形しない。`Aurora` は専用の Three.js/WebGL2 シェーダーで低域から高域までの5帯域を連続した色・エネルギーマップへ補間し、発光する上端、半透明の面光、縦フィラメント、縦横のカラーグラデーション、薄い bloom へ周波数履歴を重ねる。`Starfield` は専用シェーダーで5層の星を消失点から放射し、長い光跡、星間ダスト、色付きハロー、中心フレア、衝撃波リングを合成してワープ航行の疾走感を表す。周波数 bucket は `1,33,65,2,34,66…` 型で32方向へ散らし、各方向のエネルギーと音の立ち上がりで光跡の出現数、長さ、太さ、輝度、bloom を変える。`DNA Helix` は専用の単一パス WebGL シェーダーで、横線ごとに1時点の低音から高音までの周波数分布を表し、新しい履歴を下側へ追加して古い履歴を上側へ送る。中央・左右の二重螺旋へ霧状のカーテン、音に反応する水平光条、250〜450個相当の微粒子、40〜80本相当の短い尾、同時に2〜4本読める外向き波面を重ねる。低エネルギー時の視認性は最低輝度と履歴差分の感度で確保し、静止ノイズを音声反応として増幅しない。`Warp Hole` は左寄りの小さな暗い消失孔へ対数遠近圧縮した8群の虹色螺旋リボンを複数の奥行きで収束させ、各群11本の長く連続するstrand、狭い半透明のオーロラベールとハロー、複数の画面端から入る非対称な前景流を単一パスで合成する。20周波数ブロック×32履歴を5つの飛び石集合へ分け、各親線・strand・横断放出線を安定した集合へ分散して、energyとriseで線ごとの輝度と大小2スケールの蛇行を変える。粒子と短いスパイクは長尺線へ従属させる。ライムからローズまでの虹色は選択配色にかかわらず固有色として維持し、Themeを含む選択配色は抑制した tint に限定する。解析発光はAurora Rainbowの露出1.65とalpha 0.84を上限とし、解析ハローは輝度0.34未満へ付与せず、strength 0.31以下、radius 0.62以下に相当する寄与と広がりへ制限する。4モードとも WebGL を利用できない場合は Canvas 2D へ fallback する。
- `Warp Hole` は20周波数ブロック×32履歴を5 RGBA texel×32行の固定長DataTextureへ保持する。20ブロックは `1,6,11,16` / `2,7,12,17` / `3,8,13,18` / `4,9,14,19` / `5,10,15,20` の5集合とし、4値のmeanとpeakを混合する。各親カーテン内の中心線と左右5本ずつの11本の長いstrand、および13候補の横断放出線はfamily/line index由来の安定した別集合へ分散する。strandはlog-radius方向へ連続する大小2スケールの固有位相で緩やかに蛇行し、energyで通常輝度と揺れ、正のriseで局所輝度と揺れ幅を強める。放出線はstrandへ従属し、親core、狭いveil/shoulder/halo、8親螺旋、小さな暗い孔、非対称な奥行きを維持する。
