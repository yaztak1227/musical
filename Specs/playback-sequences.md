...
- `App.tsx`
  ...
  - プレイヤー表示用の再生元: `PlaybackSource`（`album` または `playlist` の判別可能Union）
...
- 右パネルの曲行から再生する場合、アルバム表示では表示中アルバム、プレイリスト表示では表示中プレイリストを再生元としてキューを構築する。シャッフル有効時は選択曲を先頭に保ち、同じ再生元に含まれる残りの曲だけをシャッフルする。
- `PlayerBar` のアートワーク、代替アイコン、再生元ラベル、再生元を開く操作は `PlaybackSource.type` から一貫して決定する。プレイリスト再生中も現在曲の所属アルバムはメタデータ解決に保持するが、プレイヤー表示の再生元には使用しない。
- プレイリスト由来のキューで任意のキュー曲を直接再生しても `playlistId` と `PlaybackSource.playlist` を維持し、現在曲の所属アルバムだけを更新する。
