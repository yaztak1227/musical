---
name: design-qa
description: "Strictly compare a rendered UI against a source visual target and iterate until it passes. Use after any UI addition or modification based on an image, screenshot, mockup, Figma frame, design board, or source capture, and before handing off image-to-code or visual-fidelity work. Do not use for open-ended design ideation or broad UX audits."
---
...
Do not use this skill for broad UX critique, design critique, product audits, flow reviews, or open-ended design ideation. It is a fidelity gate for work with a concrete visual target.
...
Follow the repository's `AGENTS.md`, especially its real-Tauri launch, process-ownership, and visual-verification rules. Those rules take precedence over generic browser guidance in this skill.

For image-based additions and modifications:

- Treat the supplied image, screenshot, mockup, Figma frame, design board, or source capture as the visual source of truth. Do not substitute a newly invented design system or reinterpret the source unless the user explicitly requests a redesign.
- Use the real workspace Tauri app for final visual evidence. Mock data or a non-Tauri rendering may support diagnosis but cannot satisfy the final fidelity gate.
- Never claim completion when the source target or a fresh implementation capture cannot be opened and compared. Record `final result: blocked` instead.
- Never fake visible source assets with emoji, text glyphs, CSS/div drawings, handcrafted replacement SVGs, placeholder shapes, or approximate generated imagery.

## Strict Independent Gate

The implementer must not approve its own visual work.

After each implementation capture, dispatch a fresh read-only reviewer when subagents are available. Give the reviewer the original user request, the source visual target, the fresh implementation capture, the viewport and UI state, and focused crops where details are otherwise too small. Do not give it the implementer's self-assessment or ask it to edit files.

The reviewer must:

- open and compare the source and implementation in the same comparison input;
- inspect the full composition and focused regions for typography, layout, spacing, colors, assets, icons, copy, and visible states;
- return evidence-backed P0, P1, P2, and P3 findings;
- return `passed` only when no actionable P0, P1, or P2 finding remains.

Do not average away a major mismatch with an overall similarity score. Any actionable P0, P1, or P2 finding keeps the result blocked.

When blocked, apply the fixes, capture fresh evidence from the updated build at the same viewport and state, and dispatch a fresh read-only review again. Repeat until the reviewer passes the current build or an external blocker prevents further validation. An earlier pass and stale screenshots never approve a later edit.
...
2. Capture evidence.
   ...
   - For this application, follow `AGENTS.md` to identify or launch the real workspace Tauri app safely, then capture screenshots at the intended viewport without starting or probing an ambiguous installed copy.
