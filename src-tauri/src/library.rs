...
pub fn reorder_playlist_track(
    ...
    if request.from_index != request.to_index {
        ...
        playlist_file.track_paths.insert(request.to_index, track_path);
        write_playlist_file(&playlist_path, &playlist_file)?;
    }
...
fn update_playlist_artwork_file(
    library_root: &Path,
    request: PlaylistArtworkUpdateRequest,
) -> Result<PlaylistArtworkUpdateResult, String> {
...
fn editable_playlist_file_for_id(
    library_root: &Path,
    playlist_id: &str,
...
fn read_mplaylist_file(path: &Path) -> Result<PlaylistFile, String> {
    let text = fs::read_to_string(path).map_err(to_error_string)?;
    serde_json::from_str(&text).map_err(to_error_string)
}

fn write_playlist_file(path: &Path, playlist_file: &PlaylistFile) -> Result<(), String> {
    let bytes = serde_json::to_vec_pretty(playlist_file).map_err(to_error_string)?;
    fs::write(path, bytes).map_err(to_error_string)
}

fn playlist_file_path_for_id(library_root: &Path, playlist_id: &str) -> Result<PathBuf, String> {
    let playlist_dir = playlist_dir_for_root(library_root);
...
fn unique_generated_playlist_id(playlist_dir: &Path) -> Result<String, String> {
...
fn load_all_tracks_by_playlist_path(
    library_root: &Path,
    connection: &Connection,
...
fn load_playlist_file_best_effort(library_root: &Path, path: &Path) -> Option<PlaylistFile> {
...
fn parse_m3u_playlist(library_root: &Path, path: &Path, text: &str) -> PlaylistFile {
...
fn parse_pls_playlist(library_root: &Path, path: &Path, text: &str) -> PlaylistFile {
    let mut entries = text
        ...
        .filter_map(|(key, value)| {
            ...
            Some((index, normalize_playlist_entry_path(library_root, path, value.trim())))
        })
        .collect::<Vec<_>>();
    ...
    PlaylistFile {
        ...
        name: playlist_name_from_path(path),
        artwork_path: None,
        track_paths: entries.into_iter().map(|(_, track_path)| track_path).collect(),
    }
}

fn normalize_playlist_entry_path(library_root: &Path, playlist_path: &Path, entry: &str) -> String {
...
fn playlist_id_from_path(path: &Path) -> String {
    path.file_stem()
        .and_then(|stem| stem.to_str())
...
fn playlist_name_from_path(path: &Path) -> String {
    path.file_stem()
        .and_then(|stem| stem.to_str())
        .filter(|stem| !stem.trim().is_empty())
        .map(str::to_owned)
...
fn playlist_dir_for_root(root: &Path) -> PathBuf {
    musical_dir_for_root(root).join(PLAYLIST_DIR_NAME)
}

fn playlist_track_path(library_root: &Path, file_path: &str) -> String {
    let path = Path::new(file_path);
...
fn unique_playlist_id(name: &str, playlist_dir: &Path) -> String {
...
fn sanitize_playlist_file_stem(name: &str) -> String {
    let mut stem = name
        .trim()
        .nfc()
        .collect::<String>()
...
mod tests {
    ...
    fn parses_m3u_and_pls_playlist_paths_relative_to_playlist_file() {
        ...
        let absolute = parse_m3u_playlist(
            &root,
            &m3u_path,
            &absolute_track.to_string_lossy(),
        );
    ...
    fn saves_playlist_artwork_next_to_mplaylist_file() {
        ...
        fs::write(&source_artwork_path, [0x89, b'P', b'N', b'G', 0x0d, 0x0a, 0x1a, 0x0a])
        ...
        let updated_playlist = super::read_mplaylist_file(&playlist_path).expect("read updated playlist");
